...
/* ## Fact Counter + Text Count - Our Success */
    ...
    if ($('.portfolio-slider').length) {
        ...
        // Navigate slider with mouse wheel scroll (throttled & cross-browser compatible)
        var lastScrollTime = 0;
        $('.portfolio-slider').on('wheel', function(e) {
            e.preventDefault();
            var now = new Date().getTime();
            if (now - lastScrollTime < 350) { // Throttle to prevent multiple rapid triggers
                return;
            }
            lastScrollTime = now;

            var oe = e.originalEvent;
            var delta = oe.deltaY || oe.detail || (-oe.wheelDelta);

            // Support horizontal wheel scrolling too (like on trackpads)
            if (oe.deltaX && Math.abs(oe.deltaX) > Math.abs(oe.deltaY)) {
                delta = oe.deltaX;
            }
